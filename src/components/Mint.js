...
function Mint(props) {
  ...
  const numberUp = () => {
    mintNumber < 6 ? setMintNumber(mintNumber + 1) : setMintNumber(6);
  }
  const numberDown = () => {
  ...
  const askContractToMintNft = async () => {
    ...
    setGlobalState('loading', { show: true, msg: 'Cargando...' })
    ...
    try {
        ...
        if (props.cA) {
            ...
            setAlert('Felicitaciones, haga click para ver su NFT')
        ...
        <div className="w-[50vw] h-[50vh] bg-white">Error, intente de nuevo</div>
        console.log(error)
    }
  }
